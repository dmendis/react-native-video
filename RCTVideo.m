...
-(void)setSrcs:(NSArray *)sources {
    NSLog(@"Sources delivered: %@", sources);
    ...
    for (NSDictionary *source in sources) {
        ...
        [composition insertTimeRange:timeRange ofAsset:asset atTime:insertTime error:nil];
        
        insertTime = composition.duration;
    }
    
    [self createAndExecutePlayerWithAsset:composition];
}

- (void)createAndExecutePlayerWithAsset:(AVAsset *)asset {
    ...
    [_eventDispatcher sendInputEventWithName:@"onVideoLoadStart" body:@{@"target": self.reactTag}];
}
